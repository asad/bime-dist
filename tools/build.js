#!/usr/bin/env node
/**
 * tools/build.js - BIME build pipeline (zero npm dep).
 *
...
var editorFiles = require('./editor-files.js');

var ROOT = path.join(__dirname, '..');
var EDITOR_DIR = path.join(ROOT, 'editor');
var DIST_DIR = path.join(ROOT, 'dist');
var FILES = editorFiles.FILES;
var SOURCE_SCRIPT_START = '<!-- BIME_SOURCE_SCRIPTS:start -->';
var SOURCE_SCRIPT_END = '<!-- BIME_SOURCE_SCRIPTS:end -->';

var VERSION_INFO = readVersionInfo();
var BUILD_DATE = VERSION_INFO.lastChecked;
var BIME_VERSION = VERSION_INFO.bime;

function readVersionInfo() {
    var versionsPath = path.join(ROOT, 'versions.json');
    var data = JSON.parse(fs.readFileSync(versionsPath, 'utf8'));
    if (!data || !data.bime) {
        throw new Error('versions.json must define "bime"');
    }
    if (!data.lastChecked) {
        throw new Error('versions.json must define "lastChecked" for reproducible build dates');
    }
    return data;
}
...
function rewriteDistIntegrity(html, fileName, sri) {
    var escaped = fileName.replace(/\./g, '\\.');
    var re = new RegExp(
        '(<(?:script|link)\\b[^>]*(?:src|href)="dist/' + escaped +
        '\\?v=[^"]*"[^>]*\\bintegrity=")[^"]*(")',
        'g'
    );
    return html.replace(re, '$1' + sri + '$2');
}

function makeSourceScriptBlock() {
    return SOURCE_SCRIPT_START + '\n' +
        editorFiles.scriptTags(BIME_VERSION) + '\n' +
        SOURCE_SCRIPT_END;
}

function rewriteSourceScriptBlock(page) {
    var p = path.join(ROOT, page);
    var html = fs.readFileSync(p, 'utf8');
    var start = html.indexOf(SOURCE_SCRIPT_START);
    var end = html.indexOf(SOURCE_SCRIPT_END);
    if (start < 0 || end < 0 || end < start) {
        throw new Error(page + ' missing BIME source script markers');
    }
    end += SOURCE_SCRIPT_END.length;
    var next = html.slice(0, start) + makeSourceScriptBlock() + html.slice(end);
    if (next !== html) {
        fs.writeFileSync(p, next);
    }
}

function verifySourceScriptOrder(page, files) {
    var p = path.join(ROOT, page);
    var html = fs.readFileSync(p, 'utf8');
    var scripts = [];
    ...
    if (scripts.join('\n') !== files.join('\n')) {
        throw new Error(page + ' editor script list differs from tools/editor-files.js FILES');
    }
}
...
function main() {
    ...
    // expire on cached editor/*.js. Dist bundle integrity attributes are
    // refreshed from the freshly-built bytes at the same time.
    // ---------------------------------------------------------------------
    ...
    HTML_PAGES.forEach(function (page) {
        ...
        // v1.8.14: extended pattern to include dist/ (the bundle) — without
        // this, a returning user with the old bundle URL cached but the new
        // SRI integrity attribute hits an integrity mismatch and the script
        // is blocked entirely. Match-anywhere pattern keeps editor/, js/,
        // and dist/ in lockstep.
        html = html.replace(
            /((?:src|href)="(?:editor|js|dist)\/[^"?]*\.js)(?:\?v=[^"]*)?(")/g,
            '$1?v=' + BIME_VERSION + '$2'
        );
        html = html.replace(
        ...
        html = rewriteDistIntegrity(html, 'bime.js', sriBundle);
        html = rewriteDistIntegrity(html, 'bime.min.js', sriMin);
        if (html !== before) {
            fs.writeFileSync(p, html);
        }
    });
    ['index.html', 'examples.html', 'test.html'].forEach(function (page) {
        rewriteSourceScriptBlock(page);
        verifySourceScriptOrder(page, FILES);
    });
    ...
    // Sanity: release-only strings absent.
...
// v3.0.2: expose the bundle-assembly helpers so tools/build-public.js can build
// the pathway-stripped public bundle with the EXACT same concat/minify/SRI
// pipeline (no drift, no duplication). Requiring this module has no side effects;
// the full private build runs only when invoked directly.
module.exports = {
    concat: concat,
    minify: minify,
    sha256Hex: sha256Hex,
