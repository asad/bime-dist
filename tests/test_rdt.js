...
test('SN2: CCBr.[OH-]>>CCO.[Br-] maps the 2 ethyl carbons and reports a bond change', function() {
    var r = parse('CCBr.[OH-]>>CCO.[Br-]');
    var res = RDT.mapReaction(r);
    assert.strictEqual(Object.keys(res.mapping).length, 2,
        'SN2 should map exactly the two ethyl carbons across components');
...
// =========================================================================
test('Unbalanced reaction emits a warning and best-effort 2-carbon mapping', function() {
    var r = parse('CCO>>CC');
    var res = RDT.mapReaction(r);
    ...
    // Best-effort mapping still maps both carbons.
    assert.strictEqual(Object.keys(res.mapping).length, 2);
});
