/**
 * tests/test_rdt.js - atom-atom mapping (AAM) regressions for BIME.
 *
...
test('checkBalance reports CCO>>CC=O as balanced (heavy-atom-only policy)', function() {
    // Documented BIME policy: balance is computed against heavy-atom multisets
    // only — implicit hydrogens are not counted, so even though CCO -> CC=O
    // formally loses 2 H, the heavy-atom balance is 2C+1O = 2C+1O, balanced.
    // This test pins that policy; if hydrogen-aware balance is added later,
...
test('MIN strategy on CCO>>CC=O maps all 3 heavy atoms', function() {
    ...
    assert.strictEqual(Object.keys(res.mapping).length, 3,
        'MIN should map all 3 heavy atoms');
...
test('RING strategy returns no MCS pairs on a non-ring reaction', function() {
    ...
    // yields the full 3-atom mapping in this ring-free case.
    assert.strictEqual(Object.keys(res.mapping).length, 3);
});
...
test('SN2: CCBr.[OH-]>>CCO.[Br-] maps the ethyl backbone + the incoming O and leaving Br', function() {
    var r = parse('CCBr.[OH-]>>CCO.[Br-]');
    var res = RDT.mapReaction(r);
    // v2.0.13: the charge-tolerant terminal-atom rescue now maps the
    // leaving Br (neutral C-Br -> Br(-) anion, same atom) and the
    // incoming O (O(-) hydroxide -> covalent C-O, same atom) in addition
    // to the two ethyl carbons — 4 atoms total. Pre-v2.0.13 only the two
    // ethyl carbons mapped because the formal-charge change on the
    // halide / hydroxide blocked the rescue.
    assert.strictEqual(Object.keys(res.mapping).length, 4,
        'SN2 should map the ethyl backbone (2 C) + O + Br across components');
...
test('Ar chlorination c1ccccc1.ClCl>>c1ccc(Cl)cc1.[ClH] preserves benzene ring', function() {
    ...
    // v1.4.2: leftover-atom rescue maps BOTH chlorines (one to the aryl-Cl,
    // one to the [ClH] product), so 8 atoms total are mapped (6 ring C + 2 Cl).
    // Pre-v1.4.2: only 7 (one Cl was an unpaired component).
    assert.ok(Object.keys(res.mapping).length >= 7,
        'expected >=7 atoms mapped (benzene ring + at least one Cl); got ' +
        Object.keys(res.mapping).length);
});

test('Reduction CC=O.[H][H]>>CCO maps the 3-atom C-C-O backbone', function() {
    var r = parse('CC=O.[H][H]>>CCO');
    var res = RDT.mapReaction(r);
    assert.strictEqual(Object.keys(res.mapping).length, 3,
        'expected the 3 heavy atoms (2 C + O) to be mapped');
...
// =========================================================================
test('Multiple reactants -> single product (amide formation): >=4 mapped atoms', function() {
    var r = parse('CC(=O)Cl.NCC>>CC(=O)NCC.Cl');
    var res = RDT.mapReaction(r);
    // Expect at least the acetyl C, carbonyl O, and 2 ethyl atoms to map.
    assert.ok(Object.keys(res.mapping).length >= 4,
        'expected >=4 mapped atoms; got ' + Object.keys(res.mapping).length);
    assert.ok(res.bondChanges.length >= 1, 'expected at least 1 bond change');
});

test('Single reactant -> multiple products (ether cleavage): >=3 heavy atoms map', function() {
    var r = parse('CCOCC>>CCO.CC');
    var res = RDT.mapReaction(r);
    // BIME's MIN/MIXTURE pairing maps the 3-atom CCO backbone shared by the
    // reactant and the larger product fragment. v1.4.2's leftover-atom
    // rescue extends this to also map the leaving CC fragment via mapped-
    // neighbour anchoring, giving 4-5 atoms in practice.
    assert.ok(Object.keys(res.mapping).length >= 3,
        'expected >=3 atoms (CCO backbone), rescue may add more; got ' +
        Object.keys(res.mapping).length);
});
...
// =========================================================================
test('Unbalanced reaction CCO>>CC returns status=unbalanced under v2.0.10 strict pre-check', function() {
    var r = parse('CCO>>CC');
    var res = RDT.mapReaction(r);
    // v2.0.10 default: refuse to silently map an imbalanced reaction.
    assert.strictEqual(res.status, 'unbalanced');
    assert.deepStrictEqual(res.mapping, {});
    // elementDelta is reactant - product per element. CCO>>CC loses one O.
    assert.strictEqual(res.elementDelta && res.elementDelta.O, 1,
        'expected elementDelta.O === 1 (one extra O on the reactant side); got '
        + JSON.stringify(res.elementDelta));
    var hasWarn = res.warnings && res.warnings.some(function(w) {
        return /unbalanced|balance/i.test(w);
    });
    assert.ok(hasWarn, 'expected an unbalanced warning; got ' + JSON.stringify(res.warnings));
});

test('Unbalanced reaction with requireStoichiometricBalance=false keeps legacy permissive behaviour', function() {
    var r = parse('CCO>>CC');
    var res = RDT.mapReaction(r, { requireStoichiometricBalance: false });
    // Legacy path: warns about the imbalance, still produces a best-effort
    // 2-carbon mapping.
    ...
    assert.strictEqual(Object.keys(res.mapping).length, 2);
});
...
// =========================================================================
test('mapReaction mutates the reaction so all 6 atoms carry mapNumber > 0', function() {
    var r = parse('CCO>>CC=O');
    RDT.mapReaction(r);
    // All 3 heavy atoms map on each side -> 6 total atoms with mapNumber > 0.
    assert.strictEqual(mappedAtomCount(r), 6);
});
