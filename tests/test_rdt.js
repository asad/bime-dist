...
test('Ar chlorination c1ccccc1.ClCl>>c1ccc(Cl)cc1.[ClH] preserves benzene ring', function() {
    ...
    // All 6 ring carbons + the chlorine substituent should be mapped (7 total).
    assert.strictEqual(Object.keys(res.mapping).length, 7,
        'expected 7 atoms mapped (benzene ring + Cl)');
});
...
test('Single reactant -> multiple products (ether cleavage): all 3 heavy atoms map', function() {
    var r = parse('CCOCC>>CCO.CC');
    var res = RDT.mapReaction(r);
    // BIME's MIN/MIXTURE pairing currently maps exactly the 3-atom CCO backbone
    // shared by reactant and the larger product fragment.
    assert.strictEqual(Object.keys(res.mapping).length, 3);
});
