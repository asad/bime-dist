 ...
 *   substructure  — graph-aware SMARTS matching
...
var versionInfo = require('../versions.json');
...
test('V3. failed early built-in search does not poison later library cache', function(done) {
    var saved = global.COMMON_MOLECULES;
    delete global.COMMON_MOLECULES;
    if (MolEditor.resetLibraryCache) MolEditor.resetLibraryCache();
    var ed = makeStubEditor('CCO');
    ed.searchLibrary('CCO', 'exact').then(function() {
        assert.fail('first search should reject before COMMON_MOLECULES is loaded');
    }, function(err) {
        assert.ok(/library not loaded/i.test(err.message),
            'first error should mention library; got ' + err.message);
        global.COMMON_MOLECULES = FIXTURE_LIBRARY.slice();
        return ed.searchLibrary('CCO', 'exact');
    }).then(function(hits) {
        assert.ok(hits && hits.length > 0, 'second search should use the loaded library');
        assert.strictEqual(hits[0].name, 'Ethanol');
        done && done();
    }).catch(function(e) {
        assert.fail(e);
    }).finally(function() {
...
test('V4. RDT.version stamp matches current bundle', function() {
    assert.strictEqual(typeof RDT, 'object');
    assert.strictEqual(RDT.version, versionInfo.bime);
});

test('V5. searchLibrary can yield between chunks without changing ranked results',
function(done) {
    withFixture(function() {
        var ed = makeStubEditor('c1ccccc1');
        var progressEvents = 0;
        ed.searchLibrary('c1ccccc1', 'similarity', {
            threshold: 0,
            topN: 3,
            yieldEvery: 2,
            onProgress: function(doneCount, total) {
                if (doneCount > 0 && total === FIXTURE_LIBRARY.length) progressEvents++;
            }
        }).then(function(hits) {
            assert.strictEqual(hits.length, 3);
            assert.strictEqual(hits[0].name, 'Benzene');
            assert.ok(progressEvents > 0, 'expected chunk progress callbacks');
            done && done();
        }).catch(function(e) { assert.fail(e); });
    });
});
