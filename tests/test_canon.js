 ...
 *
 * Stereo round-trip drift on stereocentres that sit in a constitutional
 * automorphism orbit is a known limitation — the chirality letters can flip
 * across re-parses while the molecule itself is preserved. See the
 * KNOWN_ORBIT_DRIFT note in test_round_trip.js.
 *
...
//
// Note: indole used to drift by a single ring-closure swap on the second
// pass — an artefact of the old input-order-dependent atom ranking. The
// v1.8.30 canonicalRank rewrite (input-order-invariant) removed that drift,
// so indole now sits in this idempotence list.
var IDEMPOTENT_CASES = [
    ...
    { name: 'indole',      smi: 'c1ccc2[nH]ccc2c1' },
...
// ---------------------------------------------------------------------------
// 7. Permutation invariance — the defining property of a canonical algorithm.
//    The SAME molecule, with its atoms created in ANY order, must canonicalise
//    to ONE string. Each molecule is rebuilt under many deterministic atom-
//    creation permutations (which scrambles every atom .id) while each atom's
//    neighbour order is preserved (so @/@@ stays valid); the writer must
//    collapse every permutation to a single form.
//
//    Regression lock for the v1.8.30 canonicalRank rewrite. The previous
//    connectivity-like ranking ignored bond orders and broke residual ties by atom
//    id, so cubane, adamantane, aspirin and EDTA each emitted several
//    different "canonical" strings depending on input atom order.
// ---------------------------------------------------------------------------
function permuteMolecule(mol, perm) {
    var out = new Molecule();
    var idMap = {};
    for (var i = 0; i < perm.length; i++) {
        var src = mol.atoms[perm[i]];
        var a = out.addAtom(src.symbol, src.x, src.y);
        a.charge = src.charge;       a.isotope = src.isotope;
        a.mapNumber = src.mapNumber; a.hydrogens = src.hydrogens;
        a.aromatic = src.aromatic;   a.chirality = src.chirality;
        a.radical = src.radical;
        idMap[src.id] = a;
    }
    // Bonds added in original order, so each atom's neighbour sequence — and
    // therefore its chirality reference frame — is preserved.
    for (var b = 0; b < mol.bonds.length; b++) {
        var bond = mol.bonds[b];
        var na = idMap[bond.atom1], nb = idMap[bond.atom2];
        if (!na || !nb) continue;
        var nbond = out.addBond(na.id, nb.id, bond.type);
        if (nbond) nbond.stereo = bond.stereo;
    }
    return out;
}
...
var PERMUTATION_CASES = [
    { name: 'cubane',      smi: 'C12C3C4C1C5C4C3C25' },
    { name: 'adamantane',  smi: 'C1C2CC3CC1CC(C2)C3' },
    { name: 'aspirin',     smi: 'CC(=O)Oc1ccccc1C(=O)O' },
    { name: 'EDTA',        smi: 'OC(=O)CN(CC(O)=O)CCN(CC(O)=O)CC(O)=O' },
    { name: 'caffeine',    smi: 'Cn1c(=O)c2c(ncn2C)n(C)c1=O' },
    { name: 'naphthalene', smi: 'c1ccc2ccccc2c1' },
    { name: 'glucose',     smi: 'OC[C@H]1O[C@@H](O)[C@H](O)[C@@H](O)[C@@H]1O' }
];

PERMUTATION_CASES.forEach(function(c) {
    test('permutation invariance: ' + c.name + ' → one canonical form over 32 atom orderings', function() {
        var base = P(c.smi);
        assert.strictEqual(base.parseErrors.length, 0,
            c.name + ' parse errors: ' + base.parseErrors.join('; '));
        var n = base.atoms.length;
        var ref = W(base);
        assert.ok(ref.length > 0, c.name + ' wrote empty string');
        for (var k = 1; k < 32; k++) {
            var got = W(permuteMolecule(base, lcgShuffle(n, k * 2654435761)));
            assert.strictEqual(got, ref,
                c.name + ' not permutation-invariant: ordering ' + k +
                ' gave "' + got + '" vs reference "' + ref + '"');
        }
    });
});
