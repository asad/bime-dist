...
test('A4. Stoichiometric mismatch (3 reactants -> 2 products) emits paletteIndex=-1', function() {
    ...
    //
    // v2.0.10: CC.OO.NN>>CO.CO is heavy-atom-unbalanced (loses both Ns).
    // The strict pre-check would short-circuit it before bipartite pairing
    // runs. This test is explicitly probing the unpaired-component path,
    // not the balance check, so opt out via requireStoichiometricBalance:false.
...
test('C3. Unpaired components get paletteIndex === -1', function() {
    // CC.OO.NN>>CO.CO is heavy-atom-unbalanced (loses both Ns); the
    // v2.0.10 strict pre-check would short-circuit it. This test is
    // explicitly probing the unpaired-component edge case, not the
    // balance check, so opt out via requireStoichiometricBalance:false.
    var r = RDT.mapReaction(parse('CC.OO.NN>>CO.CO'),
        { requireStoichiometricBalance: false });
    var unpaired = r.componentPairs.filter(function(p) { return p.paletteIndex === -1; });
