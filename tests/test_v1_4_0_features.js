...
test('A2. Two-reactant + two-product esterification yields 2 pairs', function() {
    ...
    assert.strictEqual(pairedCount(r.componentPairs), 2);
    // Distinct paletteIndex 0 and 1.
    var idxs = r.componentPairs.map(function(p) { return p.paletteIndex; }).sort();
    assert.deepStrictEqual(idxs, [0, 1]);
});

test('A3. Spectator [Na+] participates in pairing — 3 pairs', function() {
    var r = RDT.mapReaction(parse('CC(=O)O.OCC.[Na+]>>CC(=O)OCC.O.[Na+]'));
    assert.strictEqual(pairedCount(r.componentPairs), 3);
});
...
test('A4. Stoichiometric mismatch (3 reactants -> 2 products) emits paletteIndex=-1', function() {
    ...
    assert.strictEqual(unpaired.length, 1, 'expected exactly one unpaired component with paletteIndex=-1');
});
