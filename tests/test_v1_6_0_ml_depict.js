 ...
 *   5. Sanity: ML-on layout for a few less-canonical problem inputs
...
runner.test('forward pass is deterministic', function () {
    ...
    var r1 = MLDepict.predictResidual(f, nf1, nf2, nf3, 's0');
    var r2 = MLDepict.predictResidual(f, nf1, nf2, nf3, 's0');
    var r3 = MLDepict.predictResidual(f, nf1, nf2, nf3, 's0');
    assert(r1[0] === r2[0] && r2[0] === r3[0], 'dx not deterministic');
    assert(r1[1] === r2[1] && r2[1] === r3[1], 'dy not deterministic');
});

runner.test('v1.8.0: source-code one-hot changes prediction', function () {
    // Same focal + neighbour pools, different source code -> predictions
    ...
    var rS0 = MLDepict.predictResidual(f, nf1, nf2, nf3, 's0');
    var rS1 = MLDepict.predictResidual(f, nf1, nf2, nf3, 's1');
    var rS2 = MLDepict.predictResidual(f, nf1, nf2, nf3, 's2');
    var rS3 = MLDepict.predictResidual(f, nf1, nf2, nf3, 's3');
    ...
    [rS0, rS1, rS2, rS3].forEach(function (r, i) {
...
runner.test('forward pass output is finite and bounded', function () {
    ...
    for (var e = 0; e < 10; e++) {
        ...
        var r = MLDepict.predictResidual(f, nf1, nf2, nf3, 's3');
        assert(isFinite(r[0]) && isFinite(r[1]),
...
runner.test('v1.6.3/v1.8.42: toolbar layout routes groups correctly', function () {
    var fs2 = require('fs');
    var p2 = require('path');
    var src = fs2.readFileSync(p2.join(__dirname, '..', 'editor', 'MolEditor.js'), 'utf8');
    // Bonds must be row 1, Edit/View must be row 2, Custom must be bottom-left.
    ...
    var customMatch = /id:\s*'custom'[\s\S]*?placement:\s*'left'[\s\S]*?dock:\s*'bottom'/.test(src);
    ...
    if (!customMatch) throw new Error('Custom group must declare placement: left and dock: bottom');
    ...
    if (!/_leftRailBottom/.test(src)) throw new Error('_leftRailBottom not built');
...
runner.test('v1.6.2: default tool is Select (not Bond)', function () {
