 ...
 * ring systems with both N=N bridges parallel.
 *
 * As of v1.8.32 every aromatic 6-ring is laid out as an EXACT regular
 * hexagon, and the inter-ring-system bond fix (spanning-tree rigid
 * placement in pullConnectedRingSystems) brought every junction bond to
 * ~1.0x BOND_LENGTH — the overall bond-length spread dropped from 144%
 * to ~5%. What is NOT yet solved is global elongation: the molecule is
 * drawn compact/folded (bbox aspect ~1.45) rather than as a >=3:1 rod;
 * that needs the SDG elongation refiner.
 *
 * Sections:
 ...
 *
 * §2's CURRENT locks are now pinned at the TARGET (the rings genuinely
 * reach it). §3's CURRENT floor is re-pinned to the corrected, no-longer-
 * bond-stretch-inflated value; its TARGET (>=3:1 rod) stays advisory
 * until the SDG elongation refiner lands.
 *
...
var THRESHOLDS = {
    ring: {
        // v1.8.32: the rule-based layout snaps every 6-ring to an exact
        // regular hexagon (measured 0.0% / 0.0° on this dye). The old loose
        // CURRENT locks (20% / 90°) predated that snap; they are tightened
        // here to the TARGET so ring perfection is now genuinely pinned.
        currentMaxBLDev:  0.05,   // 5% — visually a regular hexagon
        currentMaxAngDev: 5,      // 5°  — same
        targetMaxBLDev:   0.05,
        targetMaxAngDev:  5
    },
    rod: {
        // v1.8.32: the inter-ring-system bond fix brought every junction
        // bond to ~1.0x BOND_LENGTH (overall bond-length spread 144% → ~5%).
        // The old 1.5 floor pinned an aspect of ~1.77 — but that 1.77 was
        // INFLATED by the 2.46x stretched biaryl bond the fix removed; the
        // correctly-bonded molecule is more compact (folded, aspect ~1.45).
        // The CURRENT floor is re-pinned to that corrected value. A true
        // ≥3:1 rod still needs the SDG elongation refiner (TARGET, advisory).
        currentMinAspect: 1.4,
        targetMinAspect:  3.0
    }
};
