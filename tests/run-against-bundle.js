...
var versionInfo = require(path.join(ROOT, 'versions.json'));
...
// including nested SDG modules.
var editorFiles = {};
function collectEditorFiles(dir) {
    fs.readdirSync(dir).forEach(function(f) {
        var p = path.join(dir, f);
        var st = fs.statSync(p);
        if (st.isDirectory()) {
            collectEditorFiles(p);
        } else if (/\.js$/.test(f)) {
            editorFiles[p] = true;
        }
    });
}
collectEditorFiles(EDITOR_DIR);
...
// Run the standard regression suite. The shim now sees the bundle in place
// of editor/*.js and editor/sdg/*.js.
var TESTS_DIR = path.join(ROOT, 'tests');
var FILES = require(path.join(TESTS_DIR, 'test-manifest.js')).FILES;
...
console.log('=== BIME v' + versionInfo.bime + ' Regression Suite (against ' + path.relative(ROOT, bundlePath) + ') ===');
console.log('');
