...
// =========================================================================
test('Hydration C=C.O>>CCO yields 1 orderChange (heavy-atom) under v1.2.0 defaults', function() {
    var res = rdt('C=C.O>>CCO');
    // Heavy-atom topology event count (excluding new v1.2.0 hydrogenChange events).
    var heavy = 0;
    for (var i = 0; i < res.bondChanges.length; i++) {
        var t = res.bondChanges[i].type;
        if (t === 'formed' || t === 'cleaved' || t === 'orderChange') { heavy++; }
    }
    assert.strictEqual(heavy, 1);
});
...
// =========================================================================
// (7) Diels-Alder: 2 heavy-atom bond changes (the C=C → C-C transitions
// become cleaved because RDT can't always pair the cycloaddition endpoints
// in v1.x).
// =========================================================================
test('Diels-Alder C=CC=C.C=C>>C1CC=CCC1 yields 2 heavy-atom bond-change events', function() {
    ...
    assert.strictEqual(heavy, 2,
        'Diels-Alder should yield exactly 2 heavy-atom bond-change events under v1.x policy');
});
