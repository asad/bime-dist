...
// =========================================================================
test('Carbonyl reduction CC=O>>CCO yields >=1 orderChange event', function() {
    var res = rdt('CC=O>>CCO');
    assert.ok(ct(res.bondChanges, 'orderChange') >= 1);
});

// =========================================================================
// (2) Hydration C=C + H2O → C-C-OH
// =========================================================================
test('Hydration C=C.O>>CCO yields >=1 bond change', function() {
...
// =========================================================================
test('Hydrogenation C=C.[H][H]>>CC yields >=1 orderChange', function() {
    var res = rdt('C=C.[H][H]>>CC');
    assert.ok(ct(res.bondChanges, 'orderChange') >= 1,
        'expected at least one C=C → C-C orderChange event');
});

// =========================================================================
// (4) Esterification — current v1.x policy emits cleaved-only events
// =========================================================================
test('Esterification CC(=O)O.OCC>>CC(=O)OCC.O yields >=1 cleaved event', function() {
    var res = rdt('CC(=O)O.OCC>>CC(=O)OCC.O');
    assert.ok(ct(res.bondChanges, 'cleaved') >= 1);
});

// =========================================================================
// (5) Hydrolysis (reverse of esterification) — same shape
// =========================================================================
test('Hydrolysis CC(=O)OCC.O>>CC(=O)O.OCC yields >=1 cleaved event', function() {
...
// (6) SN2: Br → OH
// =========================================================================
test('SN2 CCBr.[OH-]>>CCO.[Br-] yields >= 1 cleaved event', function() {
...
// =========================================================================
test('Diels-Alder C=CC=C.C=C>>C1CC=CCC1 yields >=2 events', function() {
    var res = rdt('C=CC=C.C=C>>C1CC=CCC1');
    assert.ok(res.bondChanges.length >= 2,
        'Diels-Alder should yield at least 2 bond-change events; got ' + res.bondChanges.length);
});
...
// =========================================================================
test('Isomerization CCC>>CC=C yields >=1 bond-change event', function() {
...
// =========================================================================
test('Ring-opening C1CCCCC1>>CCCCCC yields >=1 cleaved event', function() {
...
// =========================================================================
test('Ring-closing CCCCCC>>C1CCCCC1 yields >=1 formed event', function() {
    var res = rdt('CCCCCC>>C1CCCCC1');
    assert.ok(ct(res.bondChanges, 'formed') >= 1);
});
