...
test('MIN on CCO>>CC=O maps >= 2 heavy atoms', function() {
    assert.ok(mapCount(RDT.runMinPairwise(parse('CCO>>CC=O'))) >= 2);
});
...
test('RING on CCO>>CC=O maps >= 2 heavy atoms (no rings, falls back)', function() {
    assert.ok(mapCount(RDT.runRingPairwise(parse('CCO>>CC=O'))) >= 2);
});

// Reference 2: SN2 on ethyl bromide (charged ions limit MIN's pairwise reach)
test('MIN on CCBr.[OH-]>>CCO.[Br-] maps >= 2 atoms', function() {
    assert.ok(mapCount(RDT.runMinPairwise(parse('CCBr.[OH-]>>CCO.[Br-]'))) >= 2);
});
test('MAX on CCBr.[OH-]>>CCO.[Br-] maps >= 2 atoms', function() {
    assert.ok(mapCount(RDT.runMaxPairwise(parse('CCBr.[OH-]>>CCO.[Br-]'))) >= 2);
});
test('MIXTURE on CCBr.[OH-]>>CCO.[Br-] maps >= 2 atoms', function() {
    assert.ok(mapCount(RDT.runMixturePairwise(parse('CCBr.[OH-]>>CCO.[Br-]'))) >= 2);
});
test('RING on CCBr.[OH-]>>CCO.[Br-] maps >= 2 atoms (no rings)', function() {
    assert.ok(mapCount(RDT.runRingPairwise(parse('CCBr.[OH-]>>CCO.[Br-]'))) >= 2);
});

// Reference 3: aromatic chlorination — RING/MAX/MIXTURE map all 7 heavy atoms;
// MIN intentionally trims to the smallest matching component (≥1).
test('MIN on benzene chlorination maps >= 1 atom (smallest-match policy)', function() {
    assert.ok(mapCount(RDT.runMinPairwise(parse('c1ccccc1.ClCl>>c1ccc(Cl)cc1.[ClH]'))) >= 1);
});
test('MAX on benzene chlorination maps >= 6 atoms', function() {
    assert.ok(mapCount(RDT.runMaxPairwise(parse('c1ccccc1.ClCl>>c1ccc(Cl)cc1.[ClH]'))) >= 6);
});
test('MIXTURE on benzene chlorination maps >= 6 atoms', function() {
    assert.ok(mapCount(RDT.runMixturePairwise(parse('c1ccccc1.ClCl>>c1ccc(Cl)cc1.[ClH]'))) >= 6);
});
test('RING on benzene chlorination maps the 6 ring atoms', function() {
    assert.ok(mapCount(RDT.runRingPairwise(parse('c1ccccc1.ClCl>>c1ccc(Cl)cc1.[ClH]'))) >= 6);
});

// =========================================================================
// (2) Strategy comparison — all 4 produce *some* mapping for canonical reactions
// =========================================================================
test('All 4 strategies produce non-empty mappings on Diels-Alder', function() {
...
// =========================================================================
test('Disconnected reactants (CC.NN>>CCNN) produce a non-empty mapping', function() {
    var r = parse('CC.NN>>CCNN');
    var res = RDT.mapReaction(r);
    assert.ok(mapCount(res) >= 2);
});

test('Disconnected products (CCOCC>>CCO.CC) produce a non-empty mapping', function() {
...
// =========================================================================
test('[H][H]>>[H][H] produces some mapping (hydrogens treated as atoms)', function() {
    var r = parse('[H][H]>>[H][H]');
    var res = RDT.mapReaction(r);
    assert.ok(mapCount(res) >= 2);
});
...
test('Reduction CC=O.[H][H]>>CCO maps the C-O backbone', function() {
    ...
    var res = RDT.mapReaction(r);
    // At least the carbon and the oxygen heavy atoms should map.
    var heavyMapped = 0;
    for (var i = 0; i < r.atoms.length; i++) {
        if (r.atoms[i].symbol !== 'H' && r.atoms[i].mapNumber > 0) { heavyMapped++; }
    }
    assert.ok(heavyMapped >= 4, 'expected ≥4 mapped heavy atoms (2 reactant + 2 product); got ' + heavyMapped);
});
