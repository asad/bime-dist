...
// =========================================================================
test('Disconnected reactants (CC.NN>>CCNN) map all 4 atoms via leftover-atom rescue', function() {
    // v1.4.2: rescue picks up the NN reactant component (originally an
    // unpaired component because there is no separate NN product component
    // — the nitrogens ended up inside the joined CCNN product). All 4
    // reactant heavy atoms now have product partners.
    var r = parse('CC.NN>>CCNN');
    var res = RDT.mapReaction(r);
    assert.strictEqual(mapCount(res), 4);
});

test('Disconnected products (CCOCC>>CCO.CC) map >=3 reactant heavy atoms', function() {
    // v1.4.2: leftover-atom rescue catches the leaving CC fragment in
    // addition to the CCO backbone (the 3 atoms the bipartite primary pass
    // alone gets), so the mapping covers 4 atoms instead of 3.
    var r = parse('CCOCC>>CCO.CC');
    var res = RDT.mapReaction(r);
    assert.ok(mapCount(res) >= 3,
        'expected >=3 mapped atoms (CCO backbone), rescue may add more; got ' +
        mapCount(res));
});
