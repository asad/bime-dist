...
// --- Stereo ---
test('parses [C@@H](F)(Cl)Br — chirality stored on C (adjacency frame), H=1', function() {
    var m = parse('[C@@H](F)(Cl)Br');
    assert.strictEqual(m.atoms.length, 4);
    var c = m.atoms.find(function(a) { return a.symbol === 'C'; });
    // v1.8.31: the parser normalises every chirality token out of the
    // SMILES-token neighbour order ([H, F, Cl, Br] here) and into the
    // heavy-atom adjacency frame ([F, Cl, Br, H]) that SmilesWriter and
    // CIPStereo both read. Those two orders differ by an odd permutation,
    // so the raw '@@' token is stored as '@'. The external SMILES is
    // unchanged: SmilesWriter.write re-emits [C@@H](F)(Cl)Br exactly.
