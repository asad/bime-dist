...
// =========================================================================
test('sulfonamide S(=O)(=O)N in CS(=O)(=O)N → 2 matches (S=O dual symmetry)', function() {
    // The two oxygens are exchangeable, so the matcher emits two ordered
    // mappings. Locking the exact count guards against the dual-encoding
    // policy silently changing.
    assert.strictEqual(nMatches('CS(=O)(=O)N', 'S(=O)(=O)N'), 2);
});
