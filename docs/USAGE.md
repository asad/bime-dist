...
// SMARTS substructure search inside the editor
editor._promptMCS();        // open inline MCS prompt
editor._promptSimSearch();  // open inline similarity prompt
```

The workbench's **Search type** radio panel selects which mode the
in-toolbar search button defaults to: **Exact match**, **Substructure
Search**, or **Similarity Search** (≥ 0.5 Tanimoto).
