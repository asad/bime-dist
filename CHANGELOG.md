...
- **Faster cold load.** The 119 KB common-molecules library now loads
  asynchronously rather than blocking first paint, and the main bundle
  is preloaded from `<head>` so it starts downloading before the
  parser reaches the script tag. Critical-path JS dropped from
  ~852 KB to ~733 KB; workbench becomes interactive sooner.

## [1.8.20] — 2026-05-06

### Changed

- **Descriptive hover tooltips on every toolbar button.** Every action
  in the workbench now shows a clear tooltip on mouse-over explaining
  what the button does, including any keyboard shortcut.

## [1.8.19] — 2026-05-06

### Changed

- **Macro-chain linearisation.** Molecules with two or more ring
  systems connected by short acyclic chains are now rotated so the
  longest ring-system-to-ring-system path lays out horizontally,
  with a safe-revert path if neither rotation strategy improves the
  bbox aspect over the original by at least 1 %.
- **Layout maturity polish** — closes a set of small remaining
  follow-ups across the SDG step: ring-snap is bypassed only when
  the ring is already clean, fused / spiro / bridged ring placement
  is exercised in every layout pass that needs it, and the
  diagnostic suite reports a corrected SSSR-only ring clean-rate
  on the standard internal benchmark.

## [1.8.18] — 2026-05-05

### Changed

- **MacroCycleLayout wired into the default pipeline.** Rings of
  size ≥ 8 are now laid out with the egg-shape ovalisation rather
  than a plain regular polygon. Size 3–7 rings are unchanged.
- **Explicit hydrogen placement.** A new sub-phase positions any
  user-placed explicit H atoms in the 2D layout. Implicit H counts
  on heavy atoms remain untouched — those are rendered by the SVG
  depicter, not the layout. New flag: `Layout.options.placeExplicitH`
  (default `true`).
