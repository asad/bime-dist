...
(function (global) {
    ...
    function layoutComponent(mol, atomIds) {
        ...
        // -- Step 15 (v1.8.15): FUSED-RING-AWARE polygon snap ------------
        ...
        //
        // Diagnostic vs v1.8.14 on the 1,095-mol curated set
        // (tools/diagnose-ring-quality.js):
        //   - overall clean-ring rate (max/min ≤ 1.05): 68.4 % → 79.4 %
        //   - aromatic 6-rings specifically: 93.2 % clean (1825 / 1959)
        //   - 5-rings: 86.6 %; 3- and 4-rings: ~90 %
        //   - 0 atom overlaps (unchanged from v1.8.14)
        //   - the remaining ~21 % is mostly macrocycles (size ≥ 8); the
        //     MacroCycleLayout port (CDK egg-shape ovalisation, scaffolded
        //     in editor/sdg/MacroCycleLayout.js) targets ~95 %+ in a
        //     follow-up v1.8.x patch.
        // Bisazo dye benchmark: all four 6-rings are now perfect regular
        // hexagons (bond-length spread 0 %, max angle deviation 0°).
        //
        ...
        if (rings && rings.length > 0 && ringSystems && ringSystems.length > 0) {
            ...
            if (ringApplied) {
                ...
                // ONE collision sweep — no bond-relax (that would un-snap
                // the rings). The fused-ring-aware seed+reflection
                // strategy means shared edges between rings stay at exact
                // BOND_LENGTH, so the per-ring-pair distortion that v1.8.14
                // suffered is gone. resolveCollisions only adjusts NON-ring
                // (substituent) overlaps from this point.
                resolveCollisions(mol, atomIds, ringAtomSet);
            }
        }
    }
