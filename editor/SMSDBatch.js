 ...
 *   - screenAndMatch:     similarity pre-screen + exact MCS on hits
 *   - generateFingerprint: path-based molecular fingerprint
 *   - generateMcsFingerprint: MCS-aware path fingerprint
 *   - circularCounts:         count-based circular fingerprint
 *   - featureCounts:         count-based feature-class pharmacophore fingerprint
...
(function() {
    ...
    function batchSubstructure(query, targets, opts) {
        ...
        var subMatch = window.SMSDVF2;
        if (!subMatch) throw new Error('SMSDVF2 not loaded');

        var hits = [];
        for (var i = 0; i < targets.length; i++) {
            if (targets[i].n >= query.n) {
                if (subMatch.isSubstructure(query, targets[i], opts)) {
                    hits.push(i);
                }
            }
        }
    ...
    // ========================================================================
    // Screen and Match: similarity pre-screen + exact MCS on hits
    // ========================================================================

    /**
     * Two-phase pipeline:
     *   Phase 1: similarity upper-bound screening
     *   Phase 2: Exact MCS only on targets passing threshold
     *
     * @param {Object}        query        MolGraph
     * @param {Array<Object>} targets      Array of MolGraphs
     * @param {number}        [threshold=0.3] similarity threshold
     * @param {Object}        [chemOpts]   ChemOptions
     * @param {Object}        [mcsOpts]    McsOptions
     * @return {Array<Object>} Array of { targetIdx, mapping, size, tanimoto }
     */
    ...
    // ========================================================================
    // circular fingerprint — count-based
    //
    // iterative radius expansion.  Each atom starts with an
    ...
    /**
     * Generate a count-based circular fingerprint.
     *
     * @param {Object}  graph      MolGraph with .n, .neighbors[], .atomicNum[], etc.
     * @param {number}  [radius=2] neighbourhood radius (2)
     * @return {Object}            Map of {identifier: count}
     */
    function circularCounts(graph, radius) {
        if (!graph || typeof graph.n !== 'number') throw new Error('graph is required');
    ...
    // ========================================================================
    // feature-class fingerprint — count-based
    //
    // Like the circular fingerprint but initial invariants are pharmacophore feature classes:
    // donor, acceptor, positive ionisable, negative ionisable, aromatic,
    // hydrophobic — instead of atomic properties.
    // ========================================================================

    /**
     * Pharmacophore feature classification for feature-class.
    ...
    /**
     * Generate a count-based feature-class fingerprint.
     *
     * @param {Object}  graph      MolGraph
     * @param {number}  [radius=2] neighbourhood radius (2)
     * @return {Object}            Map of {identifier: count}
     */
    function featureCounts(graph, radius) {
        if (!graph || typeof graph.n !== 'number') throw new Error('graph is required');
        if (radius === undefined || radius === null) radius = 2;
        if (typeof radius !== 'number' || radius < 0) throw new Error('radius must be >= 0');
        var n = graph.n;
        ...
        // Iterative expansion (same as circular)
        var next = new Int32Array(n);
        var nbHash = new Int32Array(128);
        for (var r = 1; r <= radius; r++) {
            for (i = 0; i < n; i++) {
     ...
     * @param {string} [type='path']  Fingerprint type: 'path'|'circular'|'feature'|'torsion'|'mcs'
     * @param {Object} [opts]  Options: { radius, maxLength, fpSize }
     * @returns {Uint32Array|Object}  Binary FP (path/mcs) or count map (circular/feature/torsion)
     */
    function fingerprintFromSmiles(smiles, type, opts) {
        ...
        switch (type) {
            ...
            case 'circular':
                return circularCounts(graph, opts.radius);
            case 'feature':
                return featureCounts(graph, opts.radius);
            case 'torsion':
     ...
     * @param {Object} counts   Sparse count map from circularCounts/featureCounts/topologicalTorsion
