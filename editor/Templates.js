...
(function(global) {
    ...
    Templates.steroid = function() {
        // Build the steroid as four fused rings laid out left to right
        // Ring A (6): leftmost
        var rA = polygon(6, 0, 0);
        var atoms = rA.map(function(p) { return { symbol: 'C', x: p.x, y: p.y }; });
        var bonds = ringBonds(6, 0, false);

        // Ring B (6): shares edge 1-2 of ring A
        var shared = [atoms[1], atoms[2]];
        var ring = fusedHexagon(shared[0], shared[1]);
        var bStart = atoms.length;
        for (var i = 0; i < 4; i++) {
            atoms.push({ symbol: 'C', x: ring[i].x, y: ring[i].y });
        }
        bonds.push({ a1: 1, a2: bStart, type: SINGLE });
        ...
        bonds.push({ a1: bStart + 3, a2: 2, type: SINGLE });

        // Ring C (6): shares edge bStart+1 - bStart+2 of ring B
        shared = [atoms[bStart + 1], atoms[bStart + 2]];
        ring = fusedHexagon(shared[0], shared[1]);
        var cStart = atoms.length;
        for (var i = 0; i < 4; i++) {
            atoms.push({ symbol: 'C', x: ring[i].x, y: ring[i].y });
        }
        bonds.push({ a1: bStart + 1, a2: cStart, type: SINGLE });
        ...
        bonds.push({ a1: cStart + 3, a2: bStart + 2, type: SINGLE });

        // Ring D (5): shares edge cStart+1 - cStart+2 of ring C
        shared = [atoms[cStart + 1], atoms[cStart + 2]];
        ring = fusedPentagon(shared[0], shared[1]);
        var dStart = atoms.length;
        for (var i = 0; i < 3; i++) {
            atoms.push({ symbol: 'C', x: ring[i].x, y: ring[i].y });
        }
        bonds.push({ a1: cStart + 1, a2: dStart, type: SINGLE });
        bonds.push({ a1: dStart, a2: dStart + 1, type: SINGLE });
        bonds.push({ a1: dStart + 1, a2: dStart + 2, type: SINGLE });
        bonds.push({ a1: dStart + 2, a2: cStart + 2, type: SINGLE });
    ...
    Templates.morphinan = function() {
        ...
        // Ring B (cyclohexene): fused at edge 1-2 of A
        var ringB = fusedHexagon(atoms[1], atoms[2]);
        ...
        bonds.push({ a1: 1, a2: bStart, type: SINGLE });
        bonds.push({ a1: bStart, a2: bStart + 1, type: SINGLE });
        bonds.push({ a1: bStart + 1, a2: bStart + 2, type: DOUBLE });
        bonds.push({ a1: bStart + 2, a2: bStart + 3, type: SINGLE });
        bonds.push({ a1: bStart + 3, a2: 2, type: SINGLE });

        // Ring C (cyclohexane): fused at edge bStart+1 -- bStart+2 of B
        var ringC = fusedHexagon(atoms[bStart + 1], atoms[bStart + 2]);
        ...
        // Ring D (piperidine): fused at edge cStart+1 -- cStart+2 of C
        // N-methyl in actual morphine, here just N
        var ringD = fusedHexagon(atoms[cStart + 1], atoms[cStart + 2]);
        var dStart = atoms.length; // 14
        atoms.push({ symbol: 'C', x: ringD[0].x, y: ringD[0].y });
        atoms.push({ symbol: 'N', x: ringD[1].x, y: ringD[1].y }); // piperidine N
        atoms.push({ symbol: 'C', x: ringD[2].x, y: ringD[2].y });
        ...
        bonds.push({ a1: dStart + 3, a2: cStart + 2, type: SINGLE });

        // Oxide bridge (E ring): connects ring A atom 0 to ring B atom bStart
        // via an oxygen, forming a dihydrofuran-like 5-membered bridge
        // Bridge: atom 4 (ring A) -- O -- atom bStart (ring B)
        var oIdx = atoms.length; // 18 -> but we only have 17 target atoms
        // For classic morphinan, the ether bridge connects across A-B junction
        // Use atom 0 from A and bStart+3 from B
        atoms.push({ symbol: 'O', x: (atoms[0].x + atoms[bStart + 3].x) / 2,
                                   y: (atoms[0].y + atoms[bStart + 3].y) / 2 - BL * 0.5 });
        bonds.push({ a1: 0, a2: oIdx, type: SINGLE });
        bonds.push({ a1: oIdx, a2: bStart + 3, type: SINGLE });
    ...
    function fusedHexagon(shared1, shared2) {
        // Generate 4 new vertices for a hexagon fused to the edge shared1-shared2
        var mx = (shared1.x + shared2.x) / 2;
        var my = (shared1.y + shared2.y) / 2;
        var dx = shared2.x - shared1.x;
        var dy = shared2.y - shared1.y;
        ...
        var radius = BL / (2 * Math.sin(Math.PI / 6));
        var apothem = radius * Math.cos(Math.PI / 6);
        var cx = mx + nx * apothem;
        var cy = my + ny * apothem;

        var pts = polygon(6, cx, cy);
        var ordered = reorderToMatch(pts, shared1, shared2);
        // Return the 4 new vertices (skip the two shared ones)
        return ordered.slice(2);
    }

    function fusedPentagon(shared1, shared2) {
        var mx = (shared1.x + shared2.x) / 2;
        var my = (shared1.y + shared2.y) / 2;
        var dx = shared2.x - shared1.x;
        var dy = shared2.y - shared1.y;
        ...
        var radius = BL / (2 * Math.sin(Math.PI / 5));
        var apothem = radius * Math.cos(Math.PI / 5);
        var cx = mx + nx * apothem;
        var cy = my + ny * apothem;

        var pts = polygon(5, cx, cy);
        var ordered = reorderToMatch(pts, shared1, shared2);
        return ordered.slice(2);
    }
