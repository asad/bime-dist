 ...
 *   L2    MCS-split partition refinement (bidirectional, RRSplit maximality)
 *   L3    maximal-clique search with pivoting + k-core reduction
 *   L4    frontier extension (bond-grow + atom-frontier DFS + forced assignment)
 *   L5    Extra seeds (only when MCS-split and BK disagree)
 *
...
(function(global) {
    ...
    //   neighbors[][], label[], orbit[], canonRank[], formalCharge[],
    ...
    // -----------------------------------------------------------------------
    // Build frontier for frontier DFS
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    // Find best candidate for the frontier search
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    // frontier DFS (Level 4 — atom-frontier)
    // -----------------------------------------------------------------------
    function frontierExtDFS(g1, g2, C, cur, bestRef,
                         qNLF1, tNLF1, qNLF2, tNLF2, qNLF3, tNLF3,
                         useTwoHopNLF, useThreeHopNLF,
        ...
        if (bestQi !== -1 && bestCandCount > 1) {
            ...
            for (var i = 0; i < branchLimit; i++) {
                ...
                frontierExtDFS(g1, g2, C, cur, bestRef,
                            qNLF1, tNLF1, qNLF2, tNLF2, qNLF3, tNLF3,
                            useTwoHopNLF, useThreeHopNLF,
    ...
    // -----------------------------------------------------------------------
    // bond-grow (Level 4 — bond-oriented)
    // -----------------------------------------------------------------------
    function frontierExtBondGrow(g1, g2, C, cur, bestRef,
        ...
        if (bestQk !== -1 && bestCandCount > 1) {
            ...
            for (var bi = 0; bi < branchLimit; bi++) {
                ...
                frontierExtBondGrow(g1, g2, C, cur, bestRef,
                                 qNLF1, tNLF1, useTwoHopNLF, useThreeHopNLF,
                                 qNLF2, tNLF2, qNLF3, tNLF3,
                                 tb, localDeadline, depth + 1,
    ...
    // -----------------------------------------------------------------------
    // frontier extend entry point (Level 4)
    // -----------------------------------------------------------------------
    function frontierExtExtend(g1, g2, seed, C, tb, localMillis,
        ...
        if (mapSize(seed) > 0 && n2 >= 20) {
            ...
            frontierExtBondGrow(g1, g2, C, curCopy, bondBestRef,
                             qNLF1, tNLF1, useTwoHopNLF, useThreeHopNLF,
                             qNLF2, tNLF2, qNLF3, tNLF3,
                             tb, bondDeadline, 0,
        ...
        frontierExtDFS(g1, g2, C, curDFS, bestRef,
                    qNLF1, tNLF1, qNLF2, tNLF2, qNLF3, tNLF3,
                    useTwoHopNLF, useThreeHopNLF,
                    tb, localDeadline, 0,
                    usedQ, usedT, candBuf, bestCandBuf,
    ...
    // =======================================================================
    // L2: MCS-split partition refinement
    // =======================================================================

    // BitClass — bitset for MCS-split partition classes
    function BitClass(n) {
        this.n_bits = n;
    ...
    function partSplitRecurse(g1, g2, C, induced, tb,
        ...
        if (!branchFromTarget) {
            ...
            // Skip qi branch
            partSplitSkipVertex(g1, g2, C, induced, tb, qSets, tSets, numClasses,
                              q2t, t2q, curSize, bestQ2T, bestSizeRef, nodeCountRef,
    ...
    function refineAndRecurse(g1, g2, C, induced, tb,
        ...
        if (curSize + 1 + newUB > bestSizeRef.val) {
            partSplitRecurse(g1, g2, C, induced, tb, newQ, newT, newQ.length,
    ...
    function partSplitSkipVertex(g1, g2, C, induced, tb,
                               qSets, tSets, numClasses,
                               q2t, t2q, curSize,
                               bestQ2T, bestSizeRef, nodeCountRef,
                               n1, n2, depth, maxDepth,
        ...
        if (curSize + skipUB > bestSizeRef.val) {
            partSplitRecurse(g1, g2, C, induced, tb, skipQ, skipT, skipQ.length,
    ...
    function partSplitSeed(g1, g2, C, induced, tb) {
        var n1 = g1.n, n2 = g2.n;
        ...
        // to distinguish atoms that the base invariant cannot tell apart
        ...
        partSplitRecurse(g1, g2, C, induced, tb,
    ...
    function maxCliquePivot(R, P, X, adj, currentBestRef, N, words,
        ...
        for (w = 0; w < words; w++) {
            ...
            while (cbits) {
                ...
                newR[cv >>> 5] |= (1 << (cv & 31));
                maxCliquePivot(newR, newP, newX, adj, currentBestRef, N, words,
    ...
    function maximumCliqueSeed(g1, g2, C, induced, tb) {
        ...
        for (i = 0; i < N; i++) {
            ...
            var sig = g1.orbit[nd.qi] + '_' + g2.orbit[nd.tj] + '_'
                    + (g1.canonRank ? (g1.canonRank[nd.qi] & 0xFF) : 0) + '_'
        ...
        maxCliquePivot(R, P, X, adj, currentBestRef, N, words,
    ...
    function linearChainFastPath(g1, g2, C) {
        ...
        // DP: longest common subpath. Run it for seq2 in BOTH orientations.
        // The two chains can be stored in opposite directions (e.g. OCC vs
        // CC=O — same ethanol, O written first), and a forward-only alignment
        // is then sub-maximal (length 2 of 3, reversed). That seeds the whole
        // MCS funnel with a wrong mapping, so ethanol oxidation came out as
        // cleaved+formed (score 2) instead of a single C-O -> C=O order change
        // (score 0.5). Take the longer of forward/reverse; prefer forward on
        // ties for determinism.
        function longestCommonSubpath(s1, s2) {
            var m = s1.length, nn = s2.length, ii, jj;
            var d = new Array(m + 1);
            for (ii = 0; ii <= m; ii++) { d[ii] = arrFill(nn + 1, 0); }
            var bl = 0, bi = 0, bj = 0;
            for (ii = 1; ii <= m; ii++) {
                var x1 = s1[ii - 1];
                for (jj = 1; jj <= nn; jj++) {
                    var x2 = s2[jj - 1];
                    if (!atomsCompat(g1, x1, g2, x2, C)) { continue; }
                    if (ii > 1 && jj > 1 && d[ii - 1][jj - 1] > 0 &&
                        bondsCompat(g1, s1[ii - 2], x1, g2, s2[jj - 2], x2, C)) {
                        d[ii][jj] = d[ii - 1][jj - 1] + 1;
                    } else {
                        d[ii][jj] = 1;
                    }
                    if (d[ii][jj] > bl) { bl = d[ii][jj]; bi = ii; bj = jj; }
                }
            }
            return { len: bl, i: bi, j: bj };
        }

        var fwd = longestCommonSubpath(seq1, seq2);
        var seq2rev = seq2.slice().reverse();
        var rev = longestCommonSubpath(seq1, seq2rev);
        var use, useSeq2;
        if (rev.len > fwd.len) { use = rev; useSeq2 = seq2rev; }
        else { use = fwd; useSeq2 = seq2; }
        if (use.len <= 0) { return null; }
        var chainMCS = {};
        for (var k = 0; k < use.len; k++) {
            chainMCS[seq1[use.i - use.len + k]] = useSeq2[use.j - use.len + k];
        }
        return chainMCS;
    }
    ...
    function findMCS(g1, g2, chemOpts, mcsOpts) {
        ...
        // --- L2: MCS-split ---
        var partSplitExhaustive = false;
        var partSplitSize = 0;
        if (!tb.expired()) {
            var mcResult = partSplitSeed(g1, g2, C, M.induced, tb);
            var mcSeed = mcResult.seed;
            partSplitSize = mapSize(mcSeed);
            var mcScore = mcsScore(g1, mcSeed, M);
            if (isBetterMapping(g1, mcSeed, partSplitSize, best, bestSize, weightMode, mcScore, bestScore)) {
                best = mcSeed; bestSize = partSplitSize; bestScore = mcScore;
            }
            partSplitExhaustive = mcResult.nodeCount < 200000;
        }
        if (!weightMode && partSplitExhaustive && bestSize >= upperBound) {
            return formatResult(ppx(g1, g2, best, C, M), g1.n, g2.n);
        }
        ...
        // Report after MCS-split phase
        reportProgress(best, bestSize, g1.n, g2.n, 'partsplit');
        ...
        // --- L4: frontier extension with seeds ---
        var seeds = [];
        if (bestSize > 0) { seeds.push(mapCopy(best)); }

        // --- L5: Extra seeds (only when MCS-split and BK disagree) ---
        var skipExtraSeeds = partSplitSize > 0 && bkSize > 0 && partSplitSize === bkSize;
        ...
        for (var si = 0; si < seeds.length; si++) {
            ...
            var extSeed = ppx(g1, g2,
                frontierExtExtend(g1, g2, seeds[si], C, tb, perSeedMs,
        ...
        // Report after extension phase
        reportProgress(best, bestSize, g1.n, g2.n, 'extend');

        // Last resort: start from empty seed
        if (bestScore <= 0 && !tb.expired()) {
            best = ppx(g1, g2,
                frontierExtExtend(g1, g2, {}, C, tb, tb.remainingMs(),
                               M.useTwoHopNLF, M.useThreeHopNLF, M.connectedOnly),
                C, M);
        }
    ...
    function extractSubgraph(g, atomIndices) {
        ...
        for (k = 0; k < subN; k++) {
            ...
            sub.neighbors[k] = [];
            sub.orbit[k] = k;
            sub.canonRank[k] = g.canonRank ? g.canonRank[oldIdx] : 0;
        }
    ...
    function findAllMCS(g1, g2, chemOpts, mcsOpts, topN) {
        ...
        for (var sp = 0; sp < seedPairs.length && results.length < topN; sp++) {
            ...
            // Extend the seed via greedy + frontier extension
            ...
            // Try frontier extension if the seed produced something
            if (mapSize(extended) > 0 && !tb2.expired()) {
                var mcgResult = frontierExtExtend(g1, g2, extended, C, tb2,
