...
(function (global) {
    ...
    NonplanarBonds.assign = function (mol) {
        ...
        // Reset all single-bond stereo flags first — we re-derive from
        // scratch. Preserve double-bond stereo (E/Z, not in scope here).
        for (var b2 = 0; b2 < mol.bonds.length; b2++) {
            var bnd2 = mol.bonds[b2];
            if (bnd2.type === 1 || bnd2.type === undefined) {
                bnd2.stereo = STEREO_NONE;
            }
        }
    ...
    NonplanarBonds.assignTetrahedral = function (mol, atom, bonds, ringAtomSet, bondsByAtom) {
        ...
        // Standard: if cipLabel = 'R', point a wedge to the
        // lowest-priority neighbour (typically H) — that's the wedge.
        // If cipLabel = 'S', dash to the lowest-priority neighbour.
        var label = atom.cipLabel || (atom.chirality === '@' ? 'S' : 'R');
        bestBond.stereo = (label === 'R') ? STEREO_WEDGE : STEREO_DASH;
        // Wedge convention: the WIDE end of the wedge is at the stereo
        // centre. Make sure atom1 of the bond is `atom`.
        if (bestBond.atom1 !== atom.id) {
            // Swap orientation by inverting the wedge — the depicter
            // reads atom1→atom2 as the wedge direction.
            var tmp = bestBond.atom1;
            bestBond.atom1 = bestBond.atom2;
            bestBond.atom2 = tmp;
        }
        return 1;
    };
