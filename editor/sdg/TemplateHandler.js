...
(function (global) {
     ...
     * to matched atoms. Uses element-symbol match in linear traversal
     * order (CDK uses VF2; we use a simpler greedy that works for
     * symmetric templates).
     *
    ...
    TemplateHandler._applyTemplate = function (mol, systemAtomIds, tmpl) {
        ...
        // Greedy assignment (good enough for simple symmetric templates).
        ...
        // Apply by linear order matching same-symbol pairs.
        var usedTmpl = {};
        for (var mi = 0; mi < molAtoms.length; mi++) {
            var ma = molAtoms[mi];
            for (var ti = 0; ti < tmpl.atoms.length; ti++) {
                if (usedTmpl[ti]) continue;
                if (tmpl.atoms[ti].symbol !== ma.symbol) continue;
                ma.x = tmpl.atoms[ti].x - tmplCx;
                ma.y = tmpl.atoms[ti].y - tmplCy;
                usedTmpl[ti] = true;
                break;
            }
        }
