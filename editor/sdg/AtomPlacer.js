/**
 * editor/sdg/AtomPlacer.js — native atom-placement helpers.
 *
 * Copyright (c) 2026 BioInception PVT LTD, Cambridge, UK and Syed Asad Rahman.
 * Licensed under the Apache License, Version 2.0 - see LICENSE.txt
 *
 * AtomPlacer provides static helpers that place atoms one-at-a-time
 * relative to already-placed neighbours. It is the primary geometry
 * primitive used by StructureDiagramGenerator, RingPlacer, and
 * LayoutRefiner.
 *
 * Public API:
 ...
 *   ⚠ shouldBeLinear              — heuristic stub using degree only
 *   ⚠ getNextBondVector           — minimal implementation (perpendicular
 *                                    to the previous bond, alternating sign)
 *
 * v1.8.x patches will replace the stub methods with fuller native logic.
 */
(function (global) {
    'use strict';
    ...
    // The optional `sharedAtomsCenter` argument supports fused ring
    // systems; when null, the angular range is computed directly from
    // placed atoms.
    // ---------------------------------------------------------------------
    ...
    // n-gon centred at (cx, cy) with given radius and start angle.
    // `addAngle` lets callers flip the rotation direction.
    // ---------------------------------------------------------------------
    ...
    // ---------------------------------------------------------------------
    // shouldBeLinear — heuristic stub. Returns true if every neighbour
    // bond is triple-bonded (acetylene-like). v1.8.x will expand this
    // with fuller valence logic.
    // ---------------------------------------------------------------------
    ...
    // alternate ±60° from the previous direction. Fuller native logic pending.
    // ---------------------------------------------------------------------
