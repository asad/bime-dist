/**
 * MLDepict.js — v1.6.0-alpha tiny ML 2D-coordinate residual predictor.
 *
 * Copyright (c) 2026 BioInception PVT LTD, Cambridge, UK and Syed Asad Rahman.
 * Licensed under the Apache License, Version 2.0 - see LICENSE.txt
 *
 * Pure-JS forward pass for the v1.6.0-alpha residual coord network. Reads the
...
(function (global) {
    ...
    // ---------------------------------------------------------------------
    // Forward pass: 40 -> 64 -> 32 -> 2.
    // ---------------------------------------------------------------------
    function relu(x) { return x > 0 ? x : 0; }

    MLDepict.predictResidual = function (focalFeat, nbrPoolFeat) {
        ...
        // Build input vector (focal | nbrpool).
        var x = new Array(IN);
        for (var i = 0; i < FEATURE_DIM; i++) x[i] = focalFeat[i] || 0;
        for (var i2 = 0; i2 < FEATURE_DIM; i2++) x[FEATURE_DIM + i2] = nbrPoolFeat[i2] || 0;

        // FC1
        var h1 = new Array(H1);
        for (var j = 0; j < H1; j++) {
            var z = b1[j];
            for (var k = 0; k < IN; k++) z += x[k] * W1[k * H1 + j];
            h1[j] = relu(z);
        }
        // FC2
        var h2 = new Array(H2);
    ...
    // ---------------------------------------------------------------------
    MLDepict.refineLayout = function (mol, alpha) {
        ...
        for (var ai = 0; ai < mol.atoms.length; ai++) {
            ...
            // Neighbour centroid + neighbour-pool feature.
            var cx = 0, cy = 0;
            var nf = new Array(FEATURE_DIM);
            for (var nz = 0; nz < FEATURE_DIM; nz++) nf[nz] = 0;
            for (var k = 0; k < nbrs.length; k++) {
                var nbAtom = mol.getAtom(nbrs[k]);
                cx += nbAtom.x; cy += nbAtom.y;
                var nbf = feats[idToIdx[nbrs[k]]];
                for (var c = 0; c < FEATURE_DIM; c++) nf[c] += nbf[c];
            }
            cx /= nbrs.length; cy /= nbrs.length;
            for (var c2 = 0; c2 < FEATURE_DIM; c2++) nf[c2] /= nbrs.length;

            var r = MLDepict.predictResidual(feats[ai], nf);
