/**
 * MLDepict.js — opt-in ML 2D-coordinate residual predictor.
 *
 * Copyright (c) 2026 BioInception PVT LTD, Cambridge, UK and Syed Asad Rahman.
 * Licensed under the Apache License, Version 2.0 - see LICENSE.txt
 *
 * Pure-JS forward pass for a small residual coord network. Reads the
 ...
 *   MLDepict.featuresFor(mol, atomId, ringSet) -> feature vector
...
(function (global) {
    ...
    // ---------------------------------------------------------------------
    // Feature encoder (must match the training-time encoder).
    // ---------------------------------------------------------------------
    ...
    // ---------------------------------------------------------------------
    // Forward pass. The model auto-detects the feature width via
    // w.arch.in. Older weights remain loadable; newer weights require
    // the matching feature inputs.
    // ---------------------------------------------------------------------
    ...
    //
    // sourceTag is one of arch.sources (default: last entry / unknown
    // channel). Pass null / undefined to use the unknown channel.
    MLDepict.predictResidual = function (focalFeat, nbrPool1, nbrPool2,
        ...
        if (IN >= 4 * FD) {
            ...
            // Source one-hot at the tail. Source codes are opaque
            // identifiers; the unknown channel is the last entry.
            var sources = (w.arch && w.arch.sources) || ['s0','s1','s2','s3'];
            var unknown = sources[sources.length - 1];
            var tag = sourceTag;
